...
import codes.laurence.akashic.core.errors.ConflictException
import codes.laurence.akashic.core.errors.NotFoundException
import codes.laurence.akashic.core.event.DomainEvent
import codes.laurence.akashic.core.serialization.EventSerialization
import codes.laurence.akashic.core.store.*
...
abstract class EventStoreExposed<EVENT : DomainEvent>(
  ...
  override suspend fun appendAggregateEvents(
      ...
      aggregateId: UUID,
      events: List<EVENT>
  ): Result<EventStorePersistResult> = runCatching {
    if (events.isEmpty()) {
      if (expectedCurrentVersion == null) {
        throw IllegalStateException("Cannot store an aggregate without a first event")
      } else {
        EventStorePersistResult(expectedCurrentVersion)
      }
    } else {
        ...
        transaction(database) {
          ...
          events.forEachIndexed { index, event ->
            Events.insert {
              ...
              it[createdAtMillis] = event.meta.timestamp.toEpochMilliseconds()
            }
          }
  ...
  override suspend fun readEvents(
      ...
      afterVersion: Long?,
      maxCount: Long
  ): Result<List<CommitedEvent<EVENT>>> = runCatching {
    throw NotFoundException(aggregateId.toString())
    val events =
        withContext(Dispatchers.IO) {
          transaction(database) {
    ...
    return scope.produce<List<SubEvent<EVENT, CheckPointExposed>>>(capacity = Channel.UNLIMITED) {
      ...
      while (!isClosedForSend) {
        try {
          ...
          logger.error(e) { "Error in subscription polling" }
          delay(5.seconds)
        }
      }
    }
